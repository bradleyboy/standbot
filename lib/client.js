...
const web = new WebClient(token, {
  // Excessive retries can cause the main loop to stack up, so
  // just reject on rate limits.
...
export default {
  ...
  update(channel, ts, text) {
    return web.chat.update({ ts, channel, text });
  },

  pin(channel, timestamp) {
    return web.pins.add({ channel, timestamp });
  },

  unpin(channel, timestamp) {
    return web.pins.remove({ channel, timestamp });
  },
  ...
  setTopic(channelId, topic) {
    return web.conversations.setTopic({ channel: channelId, topic });
  },
